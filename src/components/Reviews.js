...
const Reviews = ({id, prevRating, userRated}) => {
  ...
  return (
    <div className='mt-4 border-t-2 border-gray-700 w-full'>
        ...
        />
        <button onClick={sendReview} className='bg-gray-600 flex justify-center w-full p-2'>
            {loading ? <TailSpin height={20} color="white" /> : 'Share'}
        </button>

        {reviewsLoading ? 
            <div className='mt-6 flex justify-center'><ThreeCircles height={50} color="white" /></div>
        :
        <div className='mt-4'>
            {data.map((e, i) => {
                return(
